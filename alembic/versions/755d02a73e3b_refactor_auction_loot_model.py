...
def downgrade():
    ...
    op.add_column(
        "auction",
        sa.Column("loot_id", sa.Integer, sa.ForeignKey("loot.id"), nullable=False),
    )
    op.rename_table("auction", "auction_loot")

    op.add_column("auction_session", sa.Column("price", sa.Integer))
    op.add_column(
        "auction_session",
        sa.Column("winner_id", sa.Integer, sa.ForeignKey("character.id")),
    )
