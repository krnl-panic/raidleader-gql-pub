...
def create_item_resolver(obj, info, name, boss_id, instance_id, wowhead_url):
    try:
        item = Item(name=name, boss_id=boss_id, instance_id=instance_id, wowhead_url=wowhead_url)
        db.session.add(item)
        db.session.commit()
...
@convert_kwargs_to_snake_case
def update_item_resolver(obj, info, id, name, boss_id, instance_id, wowhead_url):
    try:
